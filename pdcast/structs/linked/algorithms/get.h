...
namespace linked {
    ...
    inline auto get(const View& view, const Key& key)
    ...
    inline auto get(const View& view, const Key& key, const Value& default_value)
        -> std::enable_if_t<ViewTraits<View>::dictlike, Value&>
    {
        using Node = typename View::Node;
        Node* node = view.search(key);
    ...
    template <typename View, typename Key, typename Value>
    inline auto lru_get(View& view, const Key& key, const Value& default_value)
        -> std::enable_if_t<ViewTraits<View>::dictlike, Value&>
    {
        using Node = typename View::Node;
        Node* node = view.template search<View::Allocator::MOVE_HEAD>(key);
