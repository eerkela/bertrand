...
#     byte_order: str = "big"
# ) -> bytes:
#     """Accurate version of integer to bytes"""
#     n_bytes = (8 + (number + (number < 0)).bit_length()) // 8
#     return number.to_bytes(length=n_bytes, byteorder=byte_order, signed=True)
...
#     return int.from_bytes(buffer, byteorder=byte_order, signed=signed)
...
# TODO: rename _shorten_indices -> _shorten_list
...
def integer_to_integer(
    ...
    if is_dtype(dtype, "u"):  # unsigned integer
        min_poss = 0
...
def integer_to_decimal(
    ...
    if (pd.api.types.is_object_dtype(series) and
        not is_dtype(series, int, exact=True)):
