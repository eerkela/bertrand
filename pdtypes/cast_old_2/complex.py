...
def to_integer(series: pd.Series,
    ...
    if not pd.api.types.is_integer_dtype(dtype):
        ...
        raise TypeError(err_msg)
    series = to_float(series, force=force, tol=tol)
    return pdtypes.cast_old_2.float.to_integer(series, force=force, round=round,
                                         tol=tol, dtype=dtype)
...
def to_datetime(series: pd.Series,
    ...
    if pd.api.types.infer_dtype(series) != "complex":
        ...
        raise TypeError(err_msg)
    series = to_float(series, force=force, tol=tol)
    return pdtypes.cast_old_2.float.to_datetime(series, unit=unit, offset=offset,
                                          tz=tz)
...
def to_timedelta(series: pd.Series,
    ...
    if pd.api.types.infer_dtype(series) != "complex":
        ...
        raise TypeError(err_msg)
    series = to_float(series, force=force, tol=tol)
    return pdtypes.cast_old_2.float.to_timedelta(series, unit=unit)
