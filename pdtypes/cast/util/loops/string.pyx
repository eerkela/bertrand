...
def string_to_boolean(
    ...
    etc.) into its equivalent boolean value.  Invalid results are converted to
...
def string_to_integer(
    ...
    for i in range(arr_length):
        element = arr[i].replace(" ", "")
        try:
            result[i] = int(element, base=base)
        except ValueError as err:
            # coerce and continue
            if errors == "coerce" and base != 10:
