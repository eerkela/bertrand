...
def days_in_month(
    month: object | int | np.ndarray | pd.Series,
    year: int | np.ndarray | pd.Series = 2001
) -> int | np.ndarray | pd.Series:
    """Get the length in days of a given month, taking leap years into
    account.
    ...
    try:
        ...
        raise ValueError(f"`month` must be between 1 (January) and 12 "
                         f"(December), not {month}") from err
...
def decompose_date(
    ...
    Examples
    --------
    Dates can be any datetime type recognized by `pdtypes`:
    ...
    Or a duck type with `.year`, `.month`, and `.day` fields:
