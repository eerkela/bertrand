...
from .env.run import confirm, atomic_write_text
...
def _project_root() -> Path:
    ...
    if not root.is_dir():
        raise OSError(f"project root is not a directory: {root}")
    return root


def _render_generated_config(section: str, body: str) -> str:
    if not body.endswith("\n"):
        body += "\n"
    return (
        f"# This file is auto-generated by bertrand from [{section}] in\n"
        "# pyproject.toml. Edit pyproject.toml, not this file.\n\n"
        f"{body}"
    )


def _sync_cpp_tool_configs() -> None:
    root = _project_root()

    # load config from pyproject.toml
    with Config(root) as config:
        outputs = {
            root / CLANG_FORMAT_FILE: _render_generated_config(
                "tool.clang_format",
                config.render_clang_format(),
            ),
            root / CLANG_TIDY_FILE: _render_generated_config(
                "tool.clang_tidy",
                config.render_clang_tidy(),
            ),
            root / CLANGD_FILE: _render_generated_config(
                "tool.clangd",
                config.render_clangd(),
            ),
        }

    # check if any .clang-* files need to be updated, and skip if not
...
def _compile_commands_sources() -> tuple[Path, list[Path]]:
    compile_commands = _project_root() / "compile_commands.json"
    ...
    try:
        entries = json_parser.loads(compile_commands.read_text(encoding="utf-8"))
...
class Internal:
    ...
    def code(args: argparse.Namespace) -> None:
        ...
        with Config(_project_root()) as config:
    ...
    def check(args: argparse.Namespace) -> None:
        ...
        compile_commands, files = _compile_commands_sources()

        # run clang-tidy
        build_dir = str(compile_commands.parent.resolve())
        for source in files:
