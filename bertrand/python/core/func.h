...
public:
    ...
    pure C++ call operator as a 3-way merge between the partial arguments, default
    values, and given source arguments, provided they fulfill the enclosing signature.
    Additionally, bound arguments can be saved and encoded into a partial signature in
    a chainable fashion, using the same infrastructure to simulate a normal function
    call at every step.  Any existing partial arguments will be folded into the
    resulting signature, facilitating higher-order function composition
            ...
            static constexpr std::invoke_result_t<F, Args...> operator()(
                ...
                if constexpr (ArgTraits<T>::args()) {
                    ...
                    return []<size_t... Prev, size_t... Next>(
                        ...
                        return call<I + 1, J + 1, K + consecutive<K>>{}(
                            std::forward<decltype(parts)>(parts),
                            std::forward<decltype(defaults)>(defaults),
                            std::forward<decltype(func)>(func),
                            impl::unpack_arg<Prev>(
    ...
    /* Bind the given C++ arguments to produce a partial signature.  This operation can
    ...
    nargsf, and the true number of arguments must be extracted using
    `PyVectorcall_NARGS(nargsf)` to account for this.  Any vectorcall array that is
    produced from C++ will include this offset by default in order to speed up
    downstream code, although vectorcall arrays passed in from Python might not use it.
    If `PY_VECTORCALL_ARGUMENTS_OFFSET` is set and 'x' is written to, then it must
    always be reset to its original value before the function returns to allow for
    nested forwarding/scoping using the same argument list.
    ...
    dynamic `py::Object` types and implicitly converting them to the expected C++
    type using the same infrastructure as ordinary bertrand conversions.  This means
    that any changes made to the `__isinstance__` or `__cast__` control structs will be
    reflected here as well, and the conversion will always be as efficient as possible.
    ...
    equivalence between Bertrand wrappers and their C++ counterparts, and corresponding
    ...
    public:
        ...
        lightweight Param struct for debugging purposes. */
        ...
        separate partial arguments which will be directly merged into the C++ argument
    ...
    either Python or C++.  Uses the same format as the vectorcall arguments, meaning
    that only one conversion is needed per argument to both search the trie and invoke
    ...
    private:
        ...
        public:
            ...
            friend bool operator==(const BaseIterator& self, const BaseIterator& other) {
            ...
            friend bool operator==(const BaseIterator& self, impl::Sentinel) {
                return self.stack.empty();
            }

            friend bool operator==(impl::Sentinel, const BaseIterator& self) {
                return self.stack.empty();
            }
    ...
    public:
        Overloads(const Object& fallback) : m_fallback(fallback) {}
        Overloads(Object&& fallback) : m_fallback(std::move(fallback)) {}
        ...
        public:
            ...
            auto size() const { return wildcard.size(); }
            auto empty() const { return wildcard.empty(); }
            auto begin() const { return wildcard.begin(); }
            auto cbegin() const { return wildcard.cbegin(); }
            auto end() const { return wildcard.end(); }
            auto cend() const { return wildcard.cend(); }
        ...
        std::shared_ptr<Node> root() const {
            return m_leading_edges.begin()->target;
        }
        ...
        the trie. */
        PyObject* cache_lookup(size_t hash) const {
        ...
        /// TODO: Maybe Iterator inherits from PartialIterator?  PartialIterator
        /// will not have a vectorcall key, but will have a stack of
        /// Edges::Iterator<instance> objects, as well as the other fields.

        /// Iterator (same as PartialIterator, but uses wildcards at every level)
        /// PartialIterator (stores partials internally and substitutes where appropriate)
        /// SearchIterator (stores a full key)

        /// TODO: the only trick here is that the stack can contain wildcard
        /// iterators initialized with ellipsis, possibly interleaved with others.
        ...
        template <typename check> requires (valid_check<check>)
        struct Iterator : BaseIterator<check> {
        private:
            using Base = BaseIterator<check>;
            using Edge = Node::Edge;
            using Edges = Node::Edges;
            using Param = Vectorcall::Param;
            using Stack = std::vector<typename Edges::template Iterator<check>>;
        ...
        public:
            template <impl::inherits<Partial> P>
            Iterator(const Edges& init, size_t max_depth) :
                Base(init, max_depth)
            {
                // explore();
            }
        ...
        /* Return an iterator over all of the overloads contained in the trie, in
        topological order. */
        template <typename check = instance> requires (valid_check<check>)
        Iterator<check> begin() const {
            return {m_leading_edges, m_data};
        }

        /* Return an iterator over all of the overloads contained in the trie, in
        topological order. */
        template <typename check = instance> requires (valid_check<check>)
        Iterator<check> cbegin() const {
            return {m_leading_edges, m_data};
        }
        ...
        in topological order, as if they were being searched normally. */
        template <typename check> requires (valid_check<check>)
        struct PartialIterator : BaseIterator<check> {
        private:
            using Base = BaseIterator<check>;
            using Edge = Node::Edge;
            using Edges = Node::Edges;
        ...
        PartialIterator<check> begin(P&& partial) const {
            return {std::forward<P>(partial), m_leading_edges, m_data};
        }
        ...
        template <typename check = instance, impl::inherits<Partial> P>
            requires (valid_check<check>)
        PartialIterator<check> cbegin(P&& partial) const {
            return {std::forward<P>(partial), m_leading_edges, m_data};
        }

        impl::Sentinel end() const { return {}; }
        impl::Sentinel cend() const { return {}; }
        auto size() const { return m_data.size() - !m_data.empty(); }
        ...
        template <typename check> requires (valid_check<check>)
        struct SearchIterator : BaseIterator<check> {
        private:
            using Base = BaseIterator<check>;
            using Edge = Node::Edge;
            using Edges = Node::Edges;
        ...
        public:
            ...
            SearchIterator(V&& key, const Edges& init, const Data& data) :
                Base(init, key.size()),
        ...
        template <typename check, impl::inherits<Vectorcall> V>
            requires (valid_check<check>)
        SearchIterator<check> search(V&& key) const {
            return {std::forward<V>(key), m_leading_edges, m_data};
        }
        ...
        void clear() noexcept {
            m_data.clear();
            m_leading_edges.begin()->target.reset();
        }
    ...
    /// TODO: key() gets replaced with default constructor for Overloads::Key{}?  Or
    /// reduce it to always use inspect.signature() calls to determine the key.  Or I
    /// can have this method simply forward to the default constructor for
    /// Overloads::Key{}.
    /// -> Actually the only consistent answer is to inspect the signature dynamically
    /// via inspect.signature() call, since otherwise the Overloads::Key{} type would
    /// be specific to just this signature, and that's not appropriate.  Because of
    /// that, it may not even make sense to make this a separate class.  It could be
    /// implemented entirely in Inspect(), such that the Inspect{} object would
    /// internally store all the information the overload trie needs to insert the
    /// function and track its hash.
