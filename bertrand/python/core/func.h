...
namespace impl {
    ...
    public:
        ...
        private:
            ...
            struct _Inner<Arguments<out...>, T, Ts...> {
                ...
                using type = _Inner<typename sub_signature<T>::type, Ts...>::type;
            };
            ...
            struct _Tuple<std::tuple<out...>, I, T, Ts...> {
                ...
                using type = _Tuple<typename tuple<T>::type, I + 1, Ts...>::type;
            };
        ...
        public:
            ...
            constexpr Defaults(Values&&... values) : values([]<size_t... Js>(
        ...
        public:
            ...
            private:
                ...
                indices. */
                template <size_t I, size_t K, typename T>
                struct Entry {
                ...
                target signature, producing an unordered tuple of Values. */
                template <typename out, size_t, typename...>
                struct get_tuple { using type = out; };
                template <typename... out, size_t K, typename P, typename... Ps>
                struct get_tuple<std::tuple<out...>, K, P, Ps...> {
                    template <typename P2>
                    struct find {
                        using type = Entry<std::min(K, Arguments::args_idx), K, P2>;
                    };
                    template <typename P2> requires (impl::ArgTraits<P2>::kw())
                    struct find<P2> {
                        template <StaticStr name>
                        struct match {
                            using type = Entry<Arguments::kwargs_idx, K, P2>;
                        };
                        template <StaticStr name> requires (Arguments::template has<name>)
                        struct match<name> {
                            using type = Entry<Arguments::template idx<name>, K, P2>;
                        };
                        using type = match<impl::ArgTraits<P2>::name>::type;
                    };
                ...
                /* Reorder the arguments tuple according to the target signature to
                ...
                struct sort_tuple<std::tuple<Entries...>> {
                    template <typename out, typename>
                    struct sort { using type = out; };
                    template <typename E, typename... Es>  // initial case
                    struct sort<std::tuple<>, std::tuple<E, Es...>> {
                        using type = sort<std::tuple<E>, std::tuple<Es...>>::type;
                    };
                    template <typename T, typename... Ts, typename E, typename... Es>
                    struct sort<std::tuple<T, Ts...>, std::tuple<E, Es...>> {
                    ...
                    using type = sort<std::tuple<>, std::tuple<Entries...>>::type;
                };

                using Tuple = sort_tuple<
                    typename get_tuple<std::tuple<>, 0, Parts...>::type
                >::type;
                ...
                static constexpr bool _no_conflicting_values<I, J> = (
                    in_partial<I> || ArgTraits<Target<I>>::name == "" || !(
                        Bound::template has<ArgTraits<Target<I>>::name> && (
                            ArgTraits<Target<I>>::posonly() ||
                ...
                static constexpr bool _satisfies_required_args<I, J> = (
                    in_partial<I> || ArgTraits<Target<I>>::opt() ||
                    (
                        ArgTraits<Target<I>>::pos() &&
                            J < std::min(Bound::kw_idx, Bound::kwargs_idx)
                    ) || (
                        ArgTraits<Target<I>>::kw() &&
                            Bound::template has<ArgTraits<Target<I>>::name>
                    )
                ...
                static constexpr bool _can_convert<I, J> = [] {
                    if constexpr (ArgTraits<Target<I>>::args()) {
                        constexpr size_t source_kw =
                            std::min(Bound::kw_idx, Bound::kwargs_idx);
                        return []<size_t... Js>(std::index_sequence<Js...>) {
                            return (
                                std::convertible_to<
                                    typename ArgTraits<Source<J + Js>>::type,
                    ...
                    } else if constexpr (ArgTraits<Target<I>>::kwargs()) {
                        return []<size_t... Js>(std::index_sequence<Js...>) {
                            return (
                                (
                                    Arguments::template has<
                                        ArgTraits<Source<Bound::kw_idx + Js>>::name
                                    > || std::convertible_to<
                                        typename ArgTraits<Source<Bound::kw_idx + Js>>::type,
                                        typename ArgTraits<Target<I>>::type
                                    >
                                ) && ...
                            );
                    ...
                    } else if constexpr (ArgTraits<Source<J>>::posonly()) {
                        return std::convertible_to<
                            typename ArgTraits<Source<J>>::type,
                            typename ArgTraits<Target<I>>::type
                        > && _can_convert<I + 1, J + 1>;

                    } else if constexpr (ArgTraits<Source<J>>::kw()) {
                        constexpr StaticStr name = ArgTraits<Source<J>>::name;
                    ...
                    } else if constexpr (ArgTraits<Source<J>>::args()) {
                            ...
                            return (
                                (
                                    in_partial<I + Is> || std::convertible_to<
                                        typename ArgTraits<Source<J>>::type,
                                        typename ArgTraits<Target<I + Is>>::type
                                    >
                                ) && ...
                            );
                    ...
                    } else if constexpr (ArgTraits<Source<J>>::kwargs()) {
                        ...
                        return []<size_t... Is>(std::index_sequence<Is...>) {
                            return ((
                                in_partial<target_kw + Is> || Bound::template has<
                                    ArgTraits<Target<target_kw + Is>>::name
                                > || std::convertible_to<
                                    typename ArgTraits<Source<J>>::type,
                                    typename ArgTraits<Target<target_kw + Is>>::type
                                >
                            ) && ...);
                ...
                /* Invoking the function involves recursively inserting the partial
                arguments into the final argument pack, which is then forwarded to the
                same infrastructure as `py::call()`.  The recursion terminates when the
                first index reaches the end of the target signature, whereby both
                argument lists are guaranteed to be exhausted thanks to the prior
                template constraints. */
                template <size_t, size_t, size_t>
                struct merge {
                    template <typename D, typename F, typename P, typename... A>
                    static constexpr decltype(auto) operator()(
                        ...
                        return typename Arguments::template Bind<A...>{}(
                            std::forward<D>(defaults),
                            std::forward<F>(func),
                            std::forward<A>(args)...
                        );
                ...
                static constexpr auto build_recursive(
                    Parts... args
                ) -> std::tuple_element_t<I, Tuple> {
                    constexpr size_t idx = std::tuple_element_t<I, Tuple>::partial_idx;
                    return {impl::unpack_arg<idx>(std::forward<Parts>(args)...)};
                }

                template <size_t... Is>
                static constexpr Tuple build(std::index_sequence<Is...>, Parts... args) {
                    return {build_recursive<Is>(std::forward<Parts>(args)...)...};
                }

            public:

                explicit constexpr with_partial(Parts... args) : parts(build(
                    std::index_sequence_for<Parts...>{},
                    std::forward<Parts>(args)...
                )) {}
            ...
            static constexpr bool no_conflicting_values =
                with_partial<>::no_conflicting_values;

            static constexpr bool no_extra_positional_args =
                with_partial<>::no_extra_positional_args;

            static constexpr bool no_extra_keyword_args =
                with_partial<>::no_extra_keyword_args;

            static constexpr bool satisfies_required_args =
                with_partial<>::satisfies_required_args;

            static constexpr bool can_convert =
                with_partial<>::can_convert;
        ...
        public:
            ...
            This method uses the same semantics as `search_instance()` to find the
            target node, and will apply an `isinstance()` check to each argument to
            navigate the trie.  Note that variadic parameter packs must be expanded
            prior to calling this function.

            The `.remove()` method of `py::Function<>` will delegate to this method
            after constructing a key from the input arguments, in order to remove a
            dynamic overload from the trie. */
            template <typename Container>
            [[maybe_unused]] Object remove_instance(const Params<Container>& key) {
                assert_valid_args<instance>(key);
                return remove_node<instance>(key);
            }

            /* Equivalent to `remove_instance()`, except that the key is assumed to
            contain Python type objects rather than instances, and the trie will be
            searched by applying `issubclass()` rather than `isinstance()`.  This is
            used by the `py::Function<>` index deletion operator. */
            template <typename Container>
            [[maybe_unused]] Object remove_subclass(const Params<Container>& key) {
                assert_valid_args<subclass>(key);
                return remove_node<subclass>(key);
            }
        ...
        private:
            ...
            template <typename check, typename Container> requires (valid_check<check>)
            Object remove_node(const Params<Container>& key) {
                size_t hash;
                Object result = reinterpret_borrow<Object>(
                    recursive_search<check>(key, hash)
                );
    ...
    template <typename Sig>
    concept no_qualified_return = Sig::no_qualified_return;
    template <typename Sig>
...
concept callable =
    ...
    impl::get_signature<Func>::template Bind<Args...>::no_conflicting_values &&
    impl::get_signature<Func>::template Bind<Args...>::no_extra_positional_args &&
    impl::get_signature<Func>::template Bind<Args...>::no_extra_keyword_args &&
    impl::get_signature<Func>::template Bind<Args...>::satisfies_required_args &&
    impl::get_signature<Func>::template Bind<Args...>::can_convert;
...
template <impl::has_signature Func, typename... Args>
    requires (callable<Func, Args...> && Defaults<Func>::n == 0)
...
template <typename F = Object(*)(Arg<"args", Object>::args, Arg<"kwargs", Object>::kwargs)>
    requires (
        ...
        impl::no_qualified_return<impl::Signature<F>> &&
...
namespace impl {
    ...
    /// TODO: a separate method descriptor class may be used for regular instance
    /// methods.  That is necessary for the `&`/`|` operators to work correctly, and
    /// may also allow me to cache the deduced member function type, rather than
    /// synthesizing it in the `__get__` method.  I'm still not sure exactly how
    /// that would interact with structural typing, or if it would even be necessary/
    /// appropriate in the long run.
    /// -> I may actually not need it.  The primary concern was how to differentiate
    /// between static methods and member methods, but it's actually fairly easy if
    /// I just use GenericGetAttr to avoid invoking the descriptor protocol.  Then,
...
allows it to be used as a decorator that self-attaches to a Python class.
...
type check.)doc";
        ...
        static int __init__(
            ...
            try {
                ...
                self->func = getattr<"Function">(bertrand)(
                    reinterpret_borrow<Object>(PyTuple_GET_ITEM(args, 0))
                );
                self->member_function_type = None;
                return 0;
            } catch (...) {
                Exception::to_python();
                return -1;
        ...
        static PyObject* __call__(
            ...
            try {
                ...
                try {
                    self->member_function_type = get_member_function_type(
                        self->func,
        ...
        static PyObject* __get__(
            ...
            PyObject* cls = type == Py_None ?
                ...
                type;
            if (self->member_function_type.is(None)) {
                self->member_function_type = get_member_function_type(
                    self->func,
                    reinterpret_borrow<Object>(cls)
                );
    ...
    private:
        ...
        static Object get_member_function_type(
            ...
            Object parametrized_cls = reinterpret_borrow<Object>(
                reinterpret_cast<PyObject*>(&PyType_Type)
            )[cls];
            Object rtype = reinterpret_steal<Object>(PySlice_New(
                ptr(parametrized_cls),
                Py_None,
            ...
            for (Py_ssize_t i = 1; i < len; ++i) {
                PyTuple_SET_ITEM(
                    ptr(new_key),
                    i,
                    Py_NewRef(PyTuple_GET_ITEM(ptr(key), i - 1))
                );
            }
            return getattr<"Function">(bertrand)[new_key];
        }
        ...
        inline static PyGetSetDef getset[] = {
            {
                "__wrapped__",
                reinterpret_cast<getter>(&ClassMethod::__wrapped__),
                nullptr,
                nullptr,
                nullptr
            },
...
type check.)doc";
        ...
        static int __init__(StaticMethod* self, PyObject* args, PyObject* kwargs) noexcept {
            try {
                if (args == nullptr || PyTuple_GET_SIZE(args) != 1) {
                    PyErr_SetString(
                        PyExc_TypeError,
...
parameter is a type object, and thus the method is a class method.)doc";
        ...
        /// TODO: no idea whether this is being handled correctly.  It seems hard.
        ...
        /// TODO: add a key() method that returns an overload key constructed from the
        /// enclosing signature and translated to Python, as a compliment to overload()
        /// itself.
        ...
        /* `bool()` typically forwards to `len()`, which would cause functions to
        erroneously evaluate to false in some circumstances.  Overloading it forces
        functions to be considered truthy. */
        ...
        static PyObject* __instancecheck__(
            ...
            /// TODO: this should analyze an instance of a type, rather than the type
            /// itself.  That would allow it to match objects that transitively support
            /// the descriptor, but not permanently so.
    ...
    private:
        ...
        /* Implements the Python constructor without any type safety checks. */
        /// TODO: do stuff
        ...
        /// TODO: this gets registered as a property
