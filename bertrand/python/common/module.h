...
namespace impl {
    ...
    // TODO: Right now, this can't be rolled out as a predefined constructor, since
    // it requires that we know the module name associated with each type.  I could
    // probably require that all types implement a __module__ StaticStr, and then
    // looking up that module whenever a type object is accessed.  That would eliminate
    // this function entirely, and allow me to predefine the constructor.

    // TODO: I probably need to go back to using ModuleTag to identify modules, and
    // then just ensure that this is always done in AST bindings
    ...
    */
    ...
    struct __module__ : public BertrandTag {
    protected:
        ...
        void type() {
            ...
            // TODO: insert the type into the module's `types` dictionary, so that it
            // can be accessed via C++ template syntax.
    ...
    private:
        ...
        PyModuleObject base;
        bool initialized;
        std::unordered_map<std::type_index, PyObject*> types;
        ...
        static PyObject* __create__(PyObject* spec, PyModuleDef* def) {
            try {
                ...
                __python__ = type;  // store a reference to the type for later use
...
public:
    ...
    template <typename... Args> requires (impl::implicit_ctor<Module, Args...>)
    Module(Args&&... args) : Base(
        init_t<Module>{},
        std::forward<Args>(args)...
    ) {}

    template <typename... Args> requires (impl::explicit_ctor<Module, Args...>)
    explicit Module(Args&&... args) : Base(
        explicit_init_t<Module>{},
        std::forward<Args>(args)...
    ) {}
...
public:
    ...
    template <typename... Args> requires (impl::implicit_ctor<Type, Args...>)
    Type(Args&&... args) : Base(
        init_t<Type>{},
        std::forward<Args>(args)...
    ) {}

    template <typename... Args> requires (impl::explicit_ctor<Type, Args...>)
    explicit Type(Args&&... args) : Base(
        explicit_init_t<Type>{},
        std::forward<Args>(args)...
    ) {}
