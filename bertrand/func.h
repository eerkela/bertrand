...
inline void test() {
    constexpr def sub = [](Arg<"x", int> x, Arg<"y", int> y) {
        return *x - *y;
    };
    constexpr def div = [](Arg<"x", int> x, Arg<"y", int> y) {
        return *x / *y;
    };

    constexpr def partial = sub >> div.bind(arg<"y"> = 2) >> div.bind(arg<"y"> = 2);
    static_assert(partial(arg<"x"> = 10, arg<"y"> = 2) == 2);
}
