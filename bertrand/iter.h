...
namespace impl {
    ...
    /// TODO: mask<R, M>, which gets returned by the range's operator[] for boolean
    /// masks.
...
struct range : impl::range_tag {
    ...
    /// TODO: this should produce a special `range<impl::mask<C, M>>` type that
    /// filters the range according to the mask, and needs to be implemented
    /// separately, just after iota.
...
namespace impl {
    ...
    /// TODO: update and streamline error messages.
    ...
    /// TODO: I may need some sort of tag to detect whether a range is a sequence or
    /// not, so that the extra boolean fields can be checked reliably within
    ...
    /// TODO: it may be possible to apply a small object optimization to the
    /// iterators, whereby I would avoid an extra heap allocation if the iterator
    /// is up to, say, 24 bytes in size.
...
template <meta::not_void T> requires (meta::not_rvalue<T>)
struct sequence : range<impl::sequence<T>> {
    ...
    [[nodiscard]] explicit constexpr sequence(C&& c)
        noexcept (requires{
    ...
    /// TODO: the slicing and masking operators should be inherited like normal,
    /// but will probably need explicit forwarding to work correctly.
