...
namespace impl {
    ...
    private:
        ...
        template <size_t G, typename P>
        static constexpr bool skip(join_iterator& it, P& p)
            noexcept (G >= P::total_groups || requires(decltype((p.template get<G>())) s) {
        ...
        template <size_t G>
        struct increment {
            template <typename P, is_join_subrange S>
