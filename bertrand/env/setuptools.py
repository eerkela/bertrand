...
class CMakeLists:
    ...
    def _lib(self, target: str, ext: Extension) -> None:
        with self.path.open("a") as f:
            ...
            f.write(")\n")
            if ext.modules:
                self._modules(f, target, ext)
            if ext.include_dirs:
                self._include_dirs(f, target, ext)
            if ext.library_dirs:
                self._library_dirs(f, target, ext)
            if ext.libraries:
                self._libraries(f, target, ext)
            if ext.extra_compile_args:
                self._compile_options(f, target, ext)
            if ext.extra_link_args:
                self._link_options(f, target, ext)
            if ext.define_macros:
                self._compile_definitions(f, target, ext)

    def _exe(self, target: str, ext: Extension) -> None:
        with self.path.open("a") as f:
    ...
    def add(self, ext: Extension) -> None:
        ...
        """
        self._lib(f"{ext.name}{sysconfig.get_config_var('EXT_SUFFIX')}", ext)
        if ext.executable:
            self._exe(ext.name, ext)
...
class BuildExt(pybind11_build_ext):
    ...
    def build_extensions(self) -> None:
        ...
        conanfile = ConanFile(build_lib, self.conan)
        if conanfile:
            conanfile.install()
        ...
        subprocess.check_call(
            [
                ...
                "-G",
                "Ninja",
                str(cmakelists.path.parent),
            ],
            cwd=cmakelists.path.parent,
        ...
        try:
            subprocess.check_call(build_args, cwd=cmakelists.path.parent)
        except subprocess.CalledProcessError:
            pass  # errors are already printed to the console

        shutil.move(
            cmakelists.path.parent / "compile_commands.json",
