"""Kubernetes Dynamic Resource Allocation helpers for Bertrand devices."""
...
class DRAResourceClaimIntent:
    ...
    container_path : str | None, optional
        Future container-facing device path requested by config.
    permissions : str, optional
        Future device permissions requested by config.
    """
...
class DeviceClass:
    ...
    @classmethod
    async def upsert(cls, kube: Kube, *, timeout: float) -> Self:
        """Converge Bertrand's DRA `DeviceClass`.
...
class ResourceSlice:
    ...
    async def upsert_node_slice(
        ...
        *,
        ...
        """Publish one node's Bertrand device inventory as a ResourceSlice.
        ...
        kube : Kube
            Active Kubernetes API context.
        node_name : str
            Kubernetes node name represented by the slice.
        devices : Collection[BertrandDeviceRecord]
            Device records to expose.
        timeout : float
            Maximum request budget in seconds.
        ...
        obj = await _RESOURCE_SLICE_CLIENT.upsert(
            ...
            labels={BERTRAND_DEVICE_NODE_LABEL: _label_value(node_name)},
            timeout=timeout,
        )
        return cls._from_object(obj)
...
class ResourceClaimTemplate:
    ...
    async def create(
        ...
        *,
        namespace: str,
        intent: DRAResourceClaimIntent,
        labels: Mapping[str, str],
        timeout: float,
    ) -> Self:
        """Create one ResourceClaimTemplate for a device request.
...
async def ensure_dra_backend(
    ...
    *,
    ...
    await DeviceClass.upsert(kube, timeout=deadline - loop.time())
        ...
        pod_template=PodTemplateSpec(
            containers=[
                ContainerSpec(
                    ...
                    command=["python", "-m", "bertrand.env.kube.dra", "agent"],
                    env=[
...
def resource_claim_intents(
    *,
    ...
    container_name: str | None = None,
    metadata: Mapping[str, tuple[str | None, str]] | None = None,
    ...
    metadata : Mapping[str, tuple[str | None, str]] | None, optional
        Optional future runtime projection metadata keyed by capability ID.
    ...
    return tuple(
        DRAResourceClaimIntent(
            ...
            container_path=metadata.get(request.capability_id, (None, ""))[0],
            permissions=metadata.get(request.capability_id, (None, ""))[1],
        )
...
async def _publish_node_slice(
    ...
    *,
    ...
    await ResourceSlice.upsert_node_slice(
        kube,
        node_name=node_name,
        devices=records,
        timeout=timeout,
