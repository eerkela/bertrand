"""Legacy nerdctl network argument formatting helpers."""
...
class NetworkTableLike(Protocol):
    """Structural type for legacy network table argument emitters."""
...
def format_network(network: NetworkTableLike) -> list[str]:
    """Format legacy network configuration into nerdctl CLI flags."""
...
def format_cpus(cpus: float) -> list[str]:
    """Format legacy CPU quota into nerdctl CLI flags."""
